...
TARGET_DIR="$REPO/analysis"
TARGET_FILE="$TARGET_DIR/session_instability_analysis_2026-03-06.md"

if [ ! -d "$REPO/.git" ]; then
  echo "[!] Expected git repo not found at: $REPO" >&2
  exit 1
fi
...
echo "[i] Repo: $(pwd)"
echo "[i] Branch: $(git branch --show-current || true)"
echo "[i] Remote(s):"
git remote -v || true

git add "analysis/session_instability_analysis_2026-03-06.md"
...
git commit -m "Add Verizon session instability analysis for 2026-03-06"
git push origin main

echo
echo "[done] Wrote: $TARGET_FILE"
